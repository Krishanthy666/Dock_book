...
import { ActivitySquare, LayoutDashboard, MessageSquare, Users } from 'lucide-react';
...
const Navigation = () => {
  ...
  return (
    <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 2rem', background: 'rgba(0,0,0,0.5)', borderBottom: '1px solid var(--card-border)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 100 }}>
      <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'white', textDecoration: 'none', fontWeight: 'bold', fontSize: '1.2rem' }}>
        <ActivitySquare color="var(--accent-primary)" /> {t('logo')}
      </Link>
      <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
        <Link to="/app" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem' }}>{t('nav_checker')}</Link>
        <Link to="/community" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Users size={16} /> {t('nav_community')}
        </Link>
        <Link to="/live-chat" style={{ color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
